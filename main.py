...
# password_list = ['marketing', 'datascience']
# hashed_list = Hasher.hash_list(password_list)

# st.write(hashed_list)

with open('config.yaml') as file:
    config = yaml.load(file, Loader=SafeLoader)


# Create authenticator object (needs 5 params in v0.4.x)
authenticator = stauth.Authenticate(
    config['credentials'],
    config['cookie']['name'],
    config['cookie']['key'],
    config['cookie']['expiry_days'],
    config['preauthorized']   # ✅ required in v0.4.x
)

# Render login form (does not return values anymore)
